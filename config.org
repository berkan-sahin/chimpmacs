...
#+LANGUAGE: en
#+OPTIONS: num:nil
+The cutest+ A rather primitve and *stupid* Emacs distribution

[[./gnuchimp.png]]
* Introduction
Chimpmacs is a very simple and basic configuration file for Emacs. From here you can edit, remove, or add many aspects of Emacs you want. I encourage you to do so, because Emacs works best when it's customized to your own liking!
* Why would I use this?
#+BEGIN_QUOTE
What's the difference between a villain and a super villain? Presentation!
#+END_QUOTE
Currently, the appeal of Chimpmacs is +seeing a cute Marisa picture everytime you start up Emacs+ nothing. Also, it is a very basic config which means you can easily add, edit or remove things as you like. You can use this as a building block when making your OWN Emacs config!
...
#+END_QUOTE
As opposed to many other Emacs distributions, Chimpmacs has ZERO customization layers which means you can just jump in, look at the config file and start editing away! After all, Emacs works best when it's customized to your own liking!
* Quick install
#+BEGIN_SRC 
git clone https://github.com/berkan-sahin/chimpmacs.git ~/.emacs.d
#+END_SRC
...
*** Description
The frontend of Chimpmacs; without this there'd be no +Marisa+ badly edited monkey in your Emacs startup screen
*** Code
#+BEGIN_SRC emacs-lisp
  (use-package dashboard
    ...
    :preface
    (defun update-config ()
      "Update Chimpmacs to the latest version."
      (interactive)
      (let ((dir (expand-file-name user-emacs-directory)))
        (if (file-exists-p dir)
            (progn
              (message "Chimpmacs is updating!")
              (cd dir)
    ...
    (setq dashboard-banner-logo-title "C H I M P M A C S - Rather primitive innit?")
    (setq dashboard-startup-banner "~/.emacs.d/monkemacs.png")
    ...
    (setq dashboard-navigator-buttons
          `(;; line1
            ((,nil
              "Chimpmacs on github"
              "Open Chimpmacs' github page on your browser"
              (lambda (&rest _) (browse-url "https://github.com/berkan-sahin/chimpmacs"))
              'default)
             (nil
              "Chimpmacs crash course"
              "Open Chimpmacs' introduction to Emacs"
              (lambda (&rest _) (find-file "~/.emacs.d/Witcheat.org"))
              'default)
             (nil
              "Update Chimpmacs"
              "Get the latest Chimpmacs update. Check out the github commits for changes!"
            ...
            ((,nil
             ...
             (nil
              "Open config.org"
              "Open Chimpmacs' configuration file for easy editing"
              (lambda (&rest _) (find-file "~/.emacs.d/config.org"))
              'default)))))
#+END_SRC
*** Notes
If you pay close attention to the code in dashboard, you'll  notice that it uses custom functions defined under the :preface use-package block. I wrote all of those functions by looking at other people's Emacs distributions (Mainly [[https://github.com/seagle0128/.emacs.d][Centaur Emacs]]) and then experimenting and adapting them to Chimpmacs. If you dig around, you'll find the same things I did - maybe even more!
*** Historical
...
*** Description
Vim keybindings in Emacs. Please note that Chimpmacs has NO other evil-mode compatibility packages because I like to KISS. This might change in the future
*** Code
#+BEGIN_SRC emacs-lisp
  (use-package evil
    :ensure t
